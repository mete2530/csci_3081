...
class JsonObject {
public:
    JsonObject() : obj(*(new picojson::object())), objPointer(&obj) {}
    JsonObject(picojson::object& obj) : obj(obj), objPointer(NULL) {}
    JsonObject(const JsonObject& jsonObject) : obj(jsonObject.obj), objPointer(NULL) {}
    ~JsonObject() { delete objPointer; }
    std::vector<std::string> GetKeys();
    JsonValue operator[](const std::string& key);
    JsonObject& operator=(const JsonObject& jsonObject) { obj = jsonObject.obj; objPointer = NULL; }
    std::string ToString() const;
    const picojson::object& GetObject() const { return obj; }
private:
    picojson::object& obj;
    picojson::object* objPointer;
};
...
inline void JsonValue::operator=(double x) {
    val = picojson::value(x);
}
inline void JsonValue::operator=(float x) {
    val = picojson::value((double)x);
}
...
inline void JsonArray::Resize(int size) {
    array.resize(size);
}
